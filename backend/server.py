...
async def get_publications(
    ...
    try:
        ...
        # Use truncated abstracts instead
        for pub in top_pubs:
            # Use first 150 chars of abstract as quick summary
            abstract = pub.get("abstract", "")
            pub["ai_summary"] = abstract[:150] + "..." if len(abstract) > 150 else abstract
            pub["ai_summarized"] = False  # Mark as not AI summarized
...
async def search(
    ...
    # Skip AI summarization for speed - use truncated descriptions instead
...
# ============================================================================
# TREATMENT ADVISOR AI ENDPOINTS (AskCura)
# ============================================================================

from treatment_advisor import get_treatment_advisor
...
@api_router.post("/treatment-advisor/chat")
async def treatment_advisor_chat(
    request: ChatMessageRequest,
    session_token: Optional[str] = Cookie(None),
    authorization: Optional[str] = Header(None)
):
    """
    Chat with AskCura treatment advisor AI
    """
    user = await get_current_user(session_token, authorization)
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    ...
    try:
        advisor = get_treatment_advisor()
        response = await advisor.chat_message(
        ...
        logger.error(f"Treatment advisor chat error: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@api_router.post("/treatment-advisor/suggest")
async def treatment_advisor_suggest(
    request: TreatmentSuggestionRequest,
    session_token: Optional[str] = Cookie(None),
    authorization: Optional[str] = Header(None)
):
    """
    Get AI-powered treatment suggestions for a disease
    """
    user = await get_current_user(session_token, authorization)
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    ...
    try:
        advisor = get_treatment_advisor()
        suggestions = await advisor.get_treatment_suggestions(
        ...
        logger.error(f"Treatment suggestion error: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@api_router.post("/treatment-advisor/compare")
async def treatment_advisor_compare(
    ...
    try:
        advisor = get_treatment_advisor()
        comparison = await advisor.compare_treatments(
            disease=request.disease,
            treatment_names=request.treatment_names,
            session_id=request.session_id,
            patient_profile=request.patient_profile
        )
        ...
        logger.error(f"Treatment comparison error: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@api_router.get("/treatment-advisor/history/{session_id}")
async def get_treatment_advisor_history(
    session_id: str,
    session_token: Optional[str] = Cookie(None),
    authorization: Optional[str] = Header(None)
):
    """
    Get chat history for a treatment advisor session
    """
    user = await get_current_user(session_token, authorization)
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
